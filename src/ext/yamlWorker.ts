...
class PromiseAdapter<T> implements yamlService.Thenable<T> {
  ...
  constructor(
    executor: (
      resolve: (value?: T | yamlService.Thenable<T>) => void,
  ...
  public then<TResult>(
    onfulfilled?: (value: T) => TResult | yamlService.Thenable<TResult>,
    onrejected?: (reason: any) => void
  ): yamlService.Thenable<TResult> {
  ...
  public static all<T>(
    values: yamlService.Thenable<T>[]
  ): yamlService.Thenable<T[]> {
...
function completionHelper(
  ...
  textDocumentPosition: ls.Position
) {
  ...
  if (textLine.indexOf(":") === -1) {
    ...
    if (
      ...
      if (lineOffset[linePos + 1]) {
        newText =
          document.getText().substring(0, start + (textLine.length - 1)) +
          "holder:\r\n" +
